...
public class ReadingData {
    public static void toGetData() throws Exception {                                                   //Метод получения и обработки вводимых значений
        Scanner src = new Scanner(System.in);
        // Ввод первого элемента
        String inputOne = src.next();                                                                   // считываем данные до первого пробела
        int numberOne = 0;
        if (isNumeric(inputOne)) {
            numberOne = Integer.parseInt(inputOne);                                                     // преобразование строки в число
        }
        // Ввод арифметической операции
        String arifmeticOperation = src.next();

        // Ввод второго элемента
        String inputTwo = src.next();                                                                   // считываем данные до первого пробела
        int numberTwo = 0;
        if (isNumeric(inputTwo)) {
            numberTwo = Integer.parseInt(inputTwo);                                                     // преобразование строки в число
        }
        ...
        } else if (isRomanNumber(inputTwo) && isRomanNumber(inputOne)) {                                // Переход -> к выполнению вычислений для Римских символов
            RomCalculation.calculateRomanNumbers(inputOne,arifmeticOperation,inputTwo);
        }else{
            throw new Exception("Invalid numeral");                                                     // В случае если используются разные системы счислений
        }
    }
